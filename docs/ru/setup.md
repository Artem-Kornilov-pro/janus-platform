...
Откроется окно PyQt6 с семью вкладками: **Чат**, **Документы**, **Граф**,
**Сущности**, **Learning Brain**, **Финансы**, **Юрист** (что делает каждая
...
Набор тестов (90+, покрывает Document/Graph/Learning Brain, конвейер
загрузки, MCP-инструменты включая юридические/финансовые, и чистые хелперы
фронтенда) должен проходить без доступа к Docker/LLM (используются фикстуры
и моки); скрипты в `demo/*.py` — это «живые» smoke-тесты, требующие Neo4j и
реквизитов Yandex Cloud.
