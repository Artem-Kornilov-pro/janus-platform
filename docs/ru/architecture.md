...
пункты, обязательства, риски, нормы права, судебные решения, счета, сроки,
претензии), с лёгким доменом **Finance** (`domains/finance/`) поверх него —
для расчёта НДС/УСН и отчётов по счетам.
...
| Домен Lex | `domains/lex/` (онтология — в `core/graph_brain/schema.py`) | Юридические сущности (`Party`, `Obligation`, `Risk`, `LegalNorm`, `CourtDecision`, `Invoice`, `Deadline`, `Claim`) и MCP-инструменты рисков/обязательств/сроков, обслуживающие вкладку **«Юрист»** | [graph_brain.md](graph_brain.md), [mcp_fabric.md](mcp_fabric.md) |
| Домен Finance | `domains/finance/` | Чистые функции расчёта НДС/УСН, используемые и MCP-инструментами (`calculate_vat`, `calculate_usn_tax`), и напрямую вкладкой **«Финансы»** | [mcp_fabric.md](mcp_fabric.md) |
