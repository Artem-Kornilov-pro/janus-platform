...
|---|---|---|
| `get_entity_by_label(label, name)` | `Neo4jClient.get_entity_by_label` | Find nodes of a given label (`Party`, `Obligation`, `Risk`, `LegalNorm`, etc.) whose name/title/id/code matches. |
...
Each tool opens its own `Neo4jClient` (via `_get_client()`, which reads
`NEO4J_*` env vars) and closes it when done.
